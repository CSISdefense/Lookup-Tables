...
GO
CREATE TABLE [ETL].[source_procurement_transaction](
...
GO
CREATE NONCLUSTERED INDEX [is_ErrorLogging_source_procurement_transaction_IsTransferred] ON [ETL].[source_procurement_transaction]
(
	[IsTransferred] ASC
)
...
SET ANSI_PADDING ON
GO
CREATE NONCLUSTERED INDEX [ix_errorlogging_source_procurement_transaction_detached_award_proc_unique_Fiscal_Year] ON [ETL].[source_procurement_transaction]
(
...
SET ANSI_PADDING ON
GO
CREATE UNIQUE NONCLUSTERED INDEX [ix_Errorlogging_spt_ctu_fiscal_year] ON [ETL].[source_procurement_transaction]
(
	[Fiscal_Year] ASC,
	[detached_award_proc_unique] ASC
)
...
GO
ALTER TABLE [ETL].[source_procurement_transaction] ADD  DEFAULT (getdate()) FOR [CSISCreatedDate]
GO
ALTER TABLE [ETL].[source_procurement_transaction] ADD  DEFAULT (getdate()) FOR [CSISmodifiedDate]
GO
ALTER TABLE [ETL].[source_procurement_transaction] ADD  DEFAULT ((0)) FOR [IsTransferred]
GO
