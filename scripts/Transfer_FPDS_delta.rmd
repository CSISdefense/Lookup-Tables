...
if(dir.exists("C:\\Users\\grego\\Repositories\\USAspending-local\\")){
  ...
  path<-"C:\\Users\\grego\\Repos\\USAspending-local\\"
} else if(dir.exists("C:/Users/CBarrie/OneDrive/Documents/USAspending-local/")) {
  path <- "C:/Users/CBarrie/OneDrive/Documents/USAspending-local/"   
  stop("USAspending-local dir location unknown")
}
deltadir<-"FY(All)_All_Contracts_Delta_20260108"


# EXEC ETL.SP_LastModifiedDateHistory
```

## 2) Import flat files and check for data mismatches
...
for (f in 1:length(file.list)){
  ...
  if(file.exists(file.path(path,deltadir,save_file))){
    print(c("Skipping ",deltadir,f, "because rda already exiests","%c"))
    skip
  }
...
This section uploads the monthly delta files to SQL server. Each file
takes between a half hour and two hours to upload. Separating
this from step 2 is less efficient, but means uploads will not be
interrupted mid process by the need to fix column mismatches.

The 25k interval is due to hitting  OBDC upload errors with no details
