...
##3) Updates for ETL.Sp_FPDSdelta_6_SelectUnmatchedForeignKeys


```{r ForeignKey}
# debug(create_foreign_key_assigments)
# Skip list for select
skip_list<-c(#Contract and transaction unique identifiers handled elsewhere
             "[contract_award_unique_key]",
             "[contract_transaction_unique_key]",
             "[CSIStransactionID]",
             #Country codes handled via chain insert manually written
...
##4) Updates for ETL.SP_FPDSdelta_7_InsertAndUpdateForeignKeys

```{r InsertUnmatched}

skip_list<-c( "[CSIStransactionID]",
             "[contract_award_unique_key]",
             "[contract_transaction_unique_key]",
             #Handled via chain insert manually written
             "[legal_entity_country_code]",
             #Handled by dyad insert
             "[awardee_or_recipient_uei]",
             "[ultimate_parent_uei]",
...
#####4) Updates for SP_FPDSdelta_9_CountEmpties
```{r Empties}
...
##6) Updates for SP_FPDSdelta10_UpdateRows
```{r CompareUpdate}
...
##6) Updates for SP_FPDSdelta11_InsertRows
